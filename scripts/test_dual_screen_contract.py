...
class DualScreenContractTests(unittest.TestCase):
    ...
    def test_ext_screens_do_not_fillscreen_every_tick(self) -> None:
        ...
        blit = driver.split("static void blitStockOnce", 1)[1].split("static void ", 1)[0]
        self.assertIn("tDisplayV1ComposeCyclic(screenIndex, mElapsed, false)", blit)
        self.assertIn("ili9341ExtPushImageScaled", blit)
        self.assertIn("tDisplayV1ComposeCyclic(screenIndex, 0, false)", blit)
        ...
        # v2.4: boot-only MV|ML|BGR = 0x38 (Y via refresh/ML, not MY).
        # 0x28 = prior L/R-good upside-down; 0xAC = field FAIL.
        self.assertIn("#define ILI9341_MADCTL_ML  0x10", low)
        self.assertIn("0x38", low)
        self.assertIn("0xAC", low)
        self.assertIn(
