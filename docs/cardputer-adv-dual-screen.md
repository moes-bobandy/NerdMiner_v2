...
**Portal must not be skipped.** Dual EXT init, SD HSPI quiesce, and the mining loop must not bypass `init_WifiManager` / `NerdMinerAP`. Boot-held Enter / C / W (drain-while-held, do not `flushFifo` a still-held key) and G0 during the loading splash force the portal. After Save/Connect: always persist; **restart into mining only if `WL_CONNECTED`**. Bad password stays on `setupModeScreen` (WAITING CONFIG) — do **not** `ESP.restart()` into the `initScreen` splash. Clear `g_wantsConfig` once the portal starts this boot; splash poll is physically held keys only. Backspace-5s wipe arms a SPIFFS `/force_portal` flag so the next boot opens the portal even if SD has `config.json`. Connect-fail must **not** arm that flag. SD `config.json` remains first-boot fallback when the portal is **not** forced. Mining tasks start only after Wi‑Fi setup. No INT menu list in this firmware. Portal stays INT-only (`nerd_nav()` / `drawSetupScreen`).
...
8. **Portal Save/Connect (v1.1):** From a forced portal, Save with a **bad** password must stay on the config QR (`setupModeScreen` / WAITING CONFIG). Do **not** flash the loading/`initScreen` “Connecting to Wifi” QR, then bounce back. Save with a **good** password may splash once, then mining with a live pool (not zeros at 0.00 KH/s). Wipe / boot-hold still open the portal.
