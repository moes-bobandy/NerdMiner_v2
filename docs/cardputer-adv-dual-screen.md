...
**Portal must not be skipped.** Dual EXT init, SD HSPI quiesce, and the mining loop must not bypass `init_WifiManager` / `NerdMinerAP`. Boot-held Enter / C / W (drain-while-held, do not `flushFifo` a still-held key) and G0 during the loading splash force the portal. After Save/Connect (contract **v1.2**): persist, clear sticky `g_wantsConfig`, arm STA-first (**NVS + RTC + SPIFFS `/sta_first`**), **leave softAP**, Connecting/STA with `setEnableConfigPortal(false)`. **No `for (;;)` portal re-enter** — WiFiManager Save/Connect often returns without `shouldSaveConfig`; treating that as “no save” immediately redrew WAITING CONFIG. **Do not `ESP.restart()`**. Keyboard begin peeks STA-first **before** `drainFifo`. If `WL_CONNECTED`, proceed to mining. STA fail: Connecting + **one** retry; setup QR only if still fail after a real timeout. Sticky force-portal is **wipe-only** (`/force_portal`). Connect-fail must **not** arm `/force_portal`. SD `config.json` remains first-boot fallback when the portal is **not** forced and STA-first is **not** armed. Mining tasks start only after Wi‑Fi setup. No INT menu list in this firmware. Portal stays INT-only (`nerd_nav()` / `drawSetupScreen`).
...
8. **Portal Save/Connect (v1.2):** Save must **not** bounce to WAITING CONFIG. WM Save/Connect often ends `startConfigPortal` without `shouldSaveConfig` — do **not** `for (;;)` redraw setup. Leave AP → Connecting/STA (`enableConfigPortal(false)`). Good password → mining. STA fail → one Connecting retry, then config QR only if still fail after timeout. Leftover Enter/C/W/G0 ignored while STA-first is armed. Wipe `/force_portal` still portals next boot. Boot-hold Enter still works when STA-first is **not** armed. Do **not** arm `/force_portal` on connect fail.
