...
8. **Portal Save/Connect (v1.1):** Save from a forced portal arms `/sta_first` and restarts. Splash stays Connecting (`initScreen`) — leftover Enter/C/W/G0 must **not** reopen the config QR. Good password → mining. Bad password → config QR only after the STA connect timeout. Wipe `/force_portal` still opens the portal immediately. Do **not** arm `/force_portal` on connect fail.
